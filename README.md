...
这是一个面向 **电商商品图库** 的 **语义检索演示系统**：在本地把商品图片和标题等信息建成向量索引，用户打开网页后，用自然语言、商品图，或 **「图 + 文一起」** 在库里找 **看起来 / 说起来相近** 的商品，结果以瀑布流展示。

**可以把它理解成：**

- 只输入文字，例如「宽松亚麻衬衫」，会返回语义相关的服饰商品（**以文搜图**）。  
- 只上传一张参考图，会返回视觉风格、品类相近的商品（**以图搜图**）。  
- 同时上传图并输入补充描述，例如图是帽子、文字写「夏天遮阳 休闲」，系统把两种信息融合成一次检索（**图文联合检索**），并支持 **手动调节图文权重** 或 **由规则自动给权重（自适应 α）**。

**技术上一句话**：用 **中文 CLIP** 把「图」和「话」投影到同一向量空间，用 **FAISS** 在大量商品向量里快速找最近邻；不训练大模型，开箱用预训练权重 + 本地建库即可跑通。
